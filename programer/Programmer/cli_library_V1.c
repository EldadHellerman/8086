...
void cli_set_setup_function(cli_profile profile, cli_callback cb){
	profile->setup_function = cb;
}
...
char *cli_get_parameter(cli_profile profile, char *name){
	...
	return((p == NULL) ? NULL : p->parameter);
}
...
void cli_parse_from_string(cli_profile profile, char *input, int length){
	...
	printf("\ncalling functions:\n");
	*/
	if(profile->setup_function != NULL) profile->setup_function(profile);
