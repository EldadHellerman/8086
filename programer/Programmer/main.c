...
void start_serial(void){
	if(!open_terminal_flag) serial_open(com_port, baud_rate);
}

void end_serial(void){
	if(!open_terminal_flag) serial_close();
}
...
void upload(cli_profile cli_p){
	...
	do{
		...
		//if(send_command('c')) break; //////////////////////////////////////XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
		ok = true;
	}while(0);
	if(!ok){
		printf("\tERROR, cannot initiate communication\n");
...
void create_cli(void){
	...
	cli_set_setup_function(profile,cli_setup);
	...
	cli_add_function(profile, "terminal", "open the terminal", open_terminal);
	cli_add_parameter(profile, "-c", "--comport", "com port", "Sets the COM port");
	cli_add_parameter(profile, "-b", "--baudrate", "baud rate", "Sets the baud rate");
...
int main(int argc, char **args){
	...
	if(open_terminal_flag) terminal(com_port, baud_rate);
	return(EXIT_SUCCESS);
}
